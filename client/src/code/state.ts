...
export function createState(inParentComponent:any) {
    return {
        ...
        showComposeMessage: function(inType: string): void{
            switch (inType){
                case "new":
                    ...
                    messageTo: "", messageSubject:"", messageBody:"", messageFrom: config.userEmail
                    })
                    break;
                case "reply":
                    this.setState({currentView:"compose",
                ...
                case "contact":
                    ...
                    messageTo: this.state.contactEmail,
                        messageSubject:"", messageBody:"", messageFrom: config.userEmail
                    })
                    break;
            }
        }.bind(inParentComponent),
        ...
        getMessages: async function(inPath: string): Promise<void> {
            ...
            messages.forEach((inMessages: IMAP.IMessage) => {
                this.state.addMessageToList(inMessages)
            })
        }.bind(inParentComponent),
        ...
        showMessage: async function(inMessage: IMAP.IMessage): Promise<void>{
            ...
            this.state.showHidePleaseWait(false)
            this.setState({currentView: "message", messageID:inMessage.id, messageDate: inMessage.date,
            messageFrom: inMessage.from, messageTo: "", messageSubject:inMessage.subject, messageBody: mb})
        }.bind(inParentComponent),
        ...
        sendMessage: async function(): Promise<void>{
            ...
            await smtpWorker.sendMessage(this.state.messageTo,
                this.state.messageFrom, this.state.messageSubject, this.state.messageBody
                )
            this.state.showHidePleaseWait(false)
            this.setState({currentView:"welcome"})
        }.bind(inParentComponent),
